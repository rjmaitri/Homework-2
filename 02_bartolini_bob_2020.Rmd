...
<span style="color: green;">68% of each species of penguin will vary from the mean by the standard deviation values in this table. </span>
...
<span style="color: green;">The boxplot displays the quartiles, medians and outliers for the species as they are on each island. </span>
...
plot(avg_flip ~ avg_mass, data = avg_mass_flip, col = "forestgreen")

text(avg_flip ~ avg_mass, labels=Species_Island,data=avg_mass_flip, cex=0.9, font=1, pos = 4)

```

<span style="color: green;">This plot is frustrating, most of the species/island combinations recieve NA's, I believe it is a problem with NA's in the dataset throwin dplyr from numerical to logical. All I can really gather from this plot is that the Adelie penguins are more rotund on Biscoe than Dream island. </span>
...
plot(flipper_length_mm ~ body_mass_g, data = penguins, col=color_easy)
